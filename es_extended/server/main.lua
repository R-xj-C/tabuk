...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
    ...
    -- وظيفة لتوليد رقم عشوائي مكون من 10 أرقام بدون تكرار داخلي
    local function generateUniqueNumber()
        ...
        return table.concat(numbers) -- إرجاع الرقم كنص
    end

    -- وظيفة لتوليد player_id فريد
    local function generateUniquePlayerId(callback)
        local newId = generateUniqueNumber() -- توليد رقم فريد من 10 أرقام
        MySQL.Async.fetchScalar('SELECT player_id FROM users WHERE player_id = @player_id', {
            ['@player_id'] = newId
        }, function(existingId)
            if existingId then
                -- إذا كان الرقم موجود، جرب رقم جديد
                generateUniquePlayerId(callback)
            else
                -- إذا كان الرقم غير موجود، قم بإرجاعه
                callback(newId)
            end
        end)
    end

    -- تحقق إذا كان player_id موجود بالفعل
    MySQL.Async.fetchScalar('SELECT player_id FROM users WHERE identifier = @identifier', {
        ['@identifier'] = identifier
    }, function(existingPlayerId)
        if not existingPlayerId then
            -- توليد player_id عشوائي جديد
            generateUniquePlayerId(function(newPlayerId)
                -- حفظ الـ player_id الجديد في قاعدة البيانات
                MySQL.Async.execute('UPDATE users SET player_id = @player_id WHERE identifier = @identifier', {
                    ['@identifier'] = identifier,
                    ['@player_id'] = newPlayerId
                }, function(rowsChanged)
                    if rowsChanged > 0 then
                        print('Player ID saved successfully for ' .. identifier)
                    else
                        print('Failed to save Player ID for ' .. identifier)
                    end
                end)

                -- إرسال الـ player_id إلى الكلينت
                TriggerClientEvent('menu:updatePlayerID', playerId, newPlayerId)
            end)
        else
            -- إذا كان player_id موجود، أرسله مباشرة
            TriggerClientEvent('menu:updatePlayerID', playerId, existingPlayerId)
        end
    end)
end)

-- وظيفة مساعدة للتحقق من وجود القيمة داخل الجدول
