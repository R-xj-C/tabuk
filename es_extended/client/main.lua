...
------------------------------------------------------------------------------------------------------------------------
-- حلقة للتحقق من موت اللاعب ونقله بعد إحيائه
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0) -- التحقق بشكل مستمر

        local ped = PlayerPedId() -- الحصول على Ped الخاص باللاعب
		local spawnPoints = Config.DefaultSpawns
        -- التحقق من موت اللاعب
        if IsEntityDead(ped) then
            Citizen.Wait(3000) -- الانتظار لمدة 3 ثوانٍ

            -- التحقق من وجود الإحداثيات بشكل صحيح
            if spawnPoints and #spawnPoints > 0 then
                local spawn = spawnPoints[math.random(1, #spawnPoints)] -- اختيار موقع عشوائي

                -- إعادة إحياء اللاعب عند الإحداثيات المختارة
                NetworkResurrectLocalPlayer(spawn.x, spawn.y, spawn.z, spawn.heading, false, false)
                
                -- إعادة إحياء اللاعب
                ResurrectPed(ped)
                
                -- إعادة صحة اللاعب إلى الحد الأقصى
                SetEntityHealth(ped, GetEntityMaxHealth(ped))
                
                -- تنظيف الدم عن اللاعب
                ClearPedBloodDamage(ped)
                
                -- استعادة التحمل الكامل للاعب
                RestorePlayerStamina(PlayerId(), 100.0)

                -- إذا كنت تستخدم RedM، ضبط القيم الأساسية للاعب
                if IS_REDM then
                ...
                -- رسالة تأكيد بالنقل والعلاج
                -- TriggerEvent('chat:addMessage', { args = { '^2System', 'You have been healed and moved to a safe location!' } })
            else
                -- رسالة خطأ في حال عدم العثور على إحداثيات
                -- TriggerEvent('chat:addMessage', { args = { '^1Error', 'Spawn locations are not configured correctly!' } })
            end
        end
    end
end)

-- منع اللاعب من التعب أثناء الركض وتحسين المهارات
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        ...
        -- إكمال المهارات
        StatSetInt('MP0_STAMINA', 100, true)       		-- التحمل
        StatSetInt('MP0_STRENGTH', 100, true)      		-- القوة
        StatSetInt('MP0_LUNG_CAPACITY', 100, true) 		-- سعة الرئة
        StatSetInt('MP0_WHEELIE_ABILITY', 100, true) 	-- مهارة قيادة الدراجات
        StatSetInt('MP0_FLYING_ABILITY', 100, true)  	-- مهارة الطيران
        StatSetInt('MP0_SHOOTING_ABILITY', 100, true) 	-- مهارة استخدام الأسلحة
        StatSetInt('MP0_STEALTH_ABILITY', 100, true) 	-- مهارة التسلل
        StatSetInt('MP0_DRIVING_ABILITY', 100, true) 	-- مهارة القيادة
    end
end)